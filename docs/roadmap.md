...
| Handoff Resources (selected files → snapshot → transport) | **0.6.0** | **Active** — see [`.planning/active/0.6-handoff-resources.md`](../.planning/active/0.6-handoff-resources.md) |
...
| **0.6.0** | Handoff Resources | Agent-selected files → immutable snapshot + manifest; native CDP attach (production transport); task-scoped `handoff_read_file` | Snapshot at create; multi-file E2E; fail-closed on missing attach; no arbitrary filesystem MCP | Auto transport routing; inbound artifacts; ZIP as default |
| **0.7.0** | Claude host | Claude skill/hook equivalent; E2E by `taskId`; optional `files` field | Documented Claude path + clean evidence run | Marketplace / Windows |
| **0.8.0** | Result artifacts | Symmetric ChatGPT → Cursor file artifacts on submit | TBD | Binary artifacts |
...
### 0.6.0 — Handoff Resources (**active**)

**Spec:** [`.planning/active/0.6-handoff-resources.md`](../.planning/active/0.6-handoff-resources.md)
...
Portable create semantics (`clientSessionId` optional, `taskId` authoritative) shipped in the 0.5.x baseline — no separate portable-core release. See [`.planning/archive/0.6-portable-core.md`](../.planning/archive/0.6-portable-core.md).

**Partial land (2026-08-21):** `files[]` create API, manifest, task-scoped `handoff_read_file`, unit tests (`npm run test:files`).

**Remaining ship bar:** native CDP attach + chip verify, live multi-file E2E.

**Experiments (non-blocking):** MCP Resource primitive, context-pack, ZIP — [`.planning/experiments/`](../.planning/experiments/).

### 0.7.0 — Claude host

Same `taskId` contract; optional `files` from 0.6 on this host.

### 0.8.0 — Result artifacts (candidate)

Symmetric ChatGPT → Cursor artifacts on `handoff_submit_result`. Design 0.6 resource model to allow this without breaking changes.
...
- **In progress:** **0.6.0** — Handoff Resources.
- **Then:** **0.7.0** Claude host; **0.8.0** result artifacts (candidate).

## Near-term queue

1. **0.6.0** Handoff Resources (snapshot + native attach E2E).
2. Then **0.7.0** Claude host.
3. Then **0.8.0** result artifacts (candidate).
...
| Handoff Resources (snapshot + transport) | **0.6.0** |
...
|------|----------|--------|------------|
| 2026-08-29 | **0.6 = Handoff Resources**; absorb portable core; Claude → **0.7**; result artifacts → **0.8** candidate | Portable core already in codebase; consolidate file layer + transport; planning split ROADMAP/spec/experiments/ADR | 0.6 portable + 0.7 add files as separate milestones |
