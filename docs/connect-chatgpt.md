...
**Queue note:** the worker defers `TIMED_OUT` while ChatGPT is still generating, up to `DISPATCH_HARD_TIMEOUT_MS` (default 15m). After that (or if the composer is idle past `DISPATCH_APPROVAL_TIMEOUT_MS`), the worker marks **`TIMED_OUT`** and will claim the next QUEUED task once the tab is idle. A late `handoff_submit_result` on the same id is still accepted. Cursor's stop hook waits for `HANDOFF_WAIT_TIMEOUT` (keep it ≥ the hard timeout) and does not fail immediately on `TIMED_OUT`, so a late submit can still resume the agent.
