 ...
 * Application-layer entry used by gptmcp + thin scripts/create-worker.ts.
 *
...
import { ChatGptBrowser } from "../browser/chatgpt.js";
...
export async function runCreateWorker(
...
): Promise<{ ok: boolean; workerId?: string; workerUrl?: string }> {
  const config = loadConfig();
  const yes = Boolean(opts.yes);
  const skipCanary = Boolean(opts.skipCanary);
  ...
  const cdpEndpoint =
    ...
    config.cdpEndpoint ||
    "http://127.0.0.1:9222";
  const providedUrl = opts.workerUrl;
  ...
  const workerId = opts.id || nextWorkerId(existing);
  const sharedCdp =
    existing.length === 0
      ? cdpEndpoint
  ...
  if (!skipCanary) {
    ...
    try {
      ...
      return { ok: false, workerId, workerUrl };
    }
  } else {
    console.log(
      JSON.stringify({
  ...
  console.log(
    JSON.stringify({
      ...
      workerId,
      workerUrl,
      next: "gptmcp restart  (so the new actor binds)",
    })
  );

  if (detachBrowser) await detachBrowser();
  return { ok: true, workerId, workerUrl };
}
